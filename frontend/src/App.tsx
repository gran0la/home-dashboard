    ...
    <div id='app' className='w-full h-screen flex bg-[url(/fishies2highres.png)] bg-cover'>
        ...
        <div id='menu-bar' className='w-20 bg-slate-950/30 backdrop-blur-md rounded-full flex flex-col items-center p-2'>
          {
            navOptions.map(option => {
              return <div key={option} onClick={() => { setSelectedNav(option) }} className={`w-16 h-16 my-1 rounded-full ${selectedNav == option ? "bg-white/70" : "bg-white/30"}`}></div>
            })
          }
        </div>
      </div>
      ...
      <div id='right' className='flex-1 flex items-center justify-center'>
        <div id='main-section' className='bg-slate-700/30 backdrop-blur-md flex w-7/8 h-7/8 rounded-3xl p-3'>
          <div id='left' className='w-2/3 rounded-tl-2xl rounded-bl-2xl h-full bg-red-100/20 flex flex-col items-center justify-start'>
            <div id='plant-selection' className='w-full  flex gap-3 items-center justify-center'>
              {
                plantData.map((plant) => {
                  return (
